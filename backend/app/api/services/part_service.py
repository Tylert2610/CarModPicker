...
class PartService(BaseDynamoCRUDService[Part, PartCreate, PartUpdate]):
    ...
    def get_by_id(
        ...
        `GET /api/parts/{part_id}` is generated by `BaseDynamoEndpointRouter`
        over `BaseDynamoCRUDService.get_by_id`, so the 404 for a tombstone has
        to be added here rather than at a route body. The base class raises the
        same `not_found()` for a missing row, so both cases render identically.
        """
    ...
    def _matches(self, part: Part, filters: PartListFilters) -> bool:
        # Every `candidates()` branch ends in this predicate, so excluding
        # tombstoned parts once keeps them out of every part listing and filter
        # option, whichever index the branch above reached for.
