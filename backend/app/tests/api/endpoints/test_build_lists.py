...
def test_read_build_list_not_found(client: TestClient, db_session: Session):
    response = client.get(
        f"{settings.API_STR}/build-lists/999999"
...
def test_update_build_list_unauthenticated(client: TestClient, db_session: Session):
    ...
    create_response = client.post(
        f"{settings.API_STR}/build-lists/", json=bl_data
    )  # Uses cookie
    assert create_response.status_code == 200
    ...
    response = client.put(
        f"{settings.API_STR}/build-lists/{bl_id}", json=update_payload
    )  # No cookie sent
    assert response.status_code == 401
...
def test_update_build_list_not_found(client: TestClient, db_session: Session):
    ...
    response = client.put(
        f"{settings.API_STR}/build-lists/777777", json=update_payload
    )  # Uses cookie
    assert response.status_code == 404
    assert response.json()["detail"] == "Build List not found"
...
def test_update_other_users_build_list_forbidden(
    ...
    create_response_a = client.post(
        f"{settings.API_STR}/build-lists/", json=bl_data_a
    )  # Uses User A's cookie
    ...
    response = client.put(
        f"{settings.API_STR}/build-lists/{bl_id_a}", json=update_payload
...
def test_delete_own_build_list_success(client: TestClient, db_session: Session):
    ...
    response = client.delete(f"{settings.API_STR}/build-lists/{bl_id}")  # Uses cookie
    assert response.status_code == 200, response.text
    ...
    get_response = client.get(f"{settings.API_STR}/build-lists/{bl_id}")
...
def test_delete_build_list_unauthenticated(client: TestClient, db_session: Session):
    ...
    response = client.delete(
        f"{settings.API_STR}/build-lists/{bl_id}"
    )  # No cookie sent
    assert response.status_code == 401
...
def test_delete_build_list_not_found(client: TestClient, db_session: Session):
    ...
    response = client.delete(
        f"{settings.API_STR}/build-lists/666666"
...
def test_delete_other_users_build_list_forbidden(
    ...
    response = client.delete(
        f"{settings.API_STR}/build-lists/{bl_id_a}"
    )  # Uses User B's cookie
    assert response.status_code == 403
...
def test_read_build_lists_by_car_success(client: TestClient, db_session: Session):
    ...
    bl_data2 = {
        "name": "RX-7 Track Build",
        "description": "For race days",
        "car_id": car_id,
    }

    response1 = client.post(f"{settings.API_STR}/build-lists/", json=bl_data1)
    assert response1.status_code == 200
    bl_id1 = response1.json()["id"]

    response2 = client.post(f"{settings.API_STR}/build-lists/", json=bl_data2)
