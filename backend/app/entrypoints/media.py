"""The media domain's entrypoint.

Image upload and serving through S3: 8 routes under `/api/images`.

Every route verifies a token. This is the first domain the plan cuts, because
it is the one that exercises Pillow's `aarch64` wheel.

Run by the image as `python -m app.entrypoints.media`. `handler` is the
