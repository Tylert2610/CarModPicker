"""The moderation domain's entrypoint.

Votes, reports and bug reports: 20 routes.

Votes and reports are polymorphic over `entity_type` and `entity_id`, so this
one domain serves moderation for every other. Seventeen routes verify a token.

Run by the image as `python -m app.entrypoints.moderation`. `handler` is the
